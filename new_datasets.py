...
def get_dataset(dataset_dir, dataset_name, task=None, transform=None):
    ...
    sentigraph_names = ['Graph-SST2', 'Graph_Twitter', 'Graph_SST5']
...
class MUTAGDataset(InMemoryDataset):
    def __init__(self, root='./datasets/MUTAG', name='MUTAG', transform=None, pre_transform=None):
        self.root = root
        self.name = name
    ...
    def raw_file_names(self):
        if self.name == 'MUTAG':
            return ['MUTAG_A.txt', 'MUTAG_graph_labels.txt', 'MUTAG_graph_indicator.txt', 'MUTAG_node_labels.txt']
        elif self.name == 'Mutagenicity':
            return ['Mutagenicity_A.txt', 'Mutagenicity_graph_labels.txt', 'Mutagenicity_graph_indicator.txt', 'Mutagenicity_node_labels.txt']
    @property
...
class BA2MotifDataset(InMemoryDataset):
    def __init__(self, root='./datasets', name='BA2Motif', transform=None, pre_transform=None):
        self.name = name
...
class SentiGraphTransform(object):
    ...
    def __call__(self, data):
        #data.edge_attr = torch.zeros(data.edge_index.shape[1], dype=torch.long)
        # integrate further transform
        if self.transform is not None:
            return self.transform(data)
        return data

def load_SeniGraph(dataset_dir='./datasets', dataset_name='Graph-SST2', transform=None):
